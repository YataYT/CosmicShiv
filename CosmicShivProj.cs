...
namespace CalamityMod.Projectiles.Melee.Shortswords
{
    public class CosmicShivProj : BaseShortswordProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (!meleeEffect && Main.rand.NextBool())
            {
                for (float k = 0f; k < MathHelper.TwoPi; k += 0.08f)
                {
                    ...
                    Color color = Color.Lerp(CosmicShivTrail.DustColors[Main.rand.Next(0, CosmicShivTrail.LightDustColors.Count)], Color.White, randomWhitingValue);  // Just for even more variety in colors idk
        ...
        {
            // Congrats on thinking of trying to PvP with this
